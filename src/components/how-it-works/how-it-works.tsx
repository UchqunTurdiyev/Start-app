...
const HowItWorks = () => {
	...
	return <>
		<SectionTitle textAlign={'center'} title={'How it works?'} subtitle={'10,000+ unique online course list'} />
		...
		<SimpleGrid mt={10} columns={5} spacing={10} alignItems={'center '}>
			{data.map((item, idx) => {
				...
				return 	<Fragment key={idx}>
					{!odd ? <Stack justify={'center'} align={'center'}>
						<Flex w={100} h={100} justify={'center'} align={'center'} backgroundColor={backgroundColor} borderRadius={'full'}>{item.icon}</Flex>
...
const data = [
	{title: 'Sign Up to Platform', icon: <OnlineCourseIcon />},
	{icon: <RightLineIcon />},
