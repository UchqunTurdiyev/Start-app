			...
			<Card>
				<CardBody p={0}>
					<HStack justify={'center'}>
						...
						<Image width={480} height={480} src='/image/draft.png' alt='instructor' />
					</HStack>
				</CardBody>
			</Card>
					...
					<TabPanels>
						<TabPanel>
							<Grid gridTemplateColumns={'1fr 1fr'} gap={4}>
								{courses.map(item => <DraftCourseCard key={item.slug} item={item} status={'Draft'} />).splice(0, 2)}
							</Grid>
						</TabPanel>
						<TabPanel>
							<Grid gridTemplateColumns={'1fr 1fr'} gap={4}>
								{courses.map(item => <DraftCourseCard key={item.slug} item={item} status={'Active'} />).splice(0, 2)}
							</Grid>
						</TabPanel>
					</TabPanels>
				</Tabs>
